...
void ArvoreAVL::insere(int chave, NoAVL *&no, bool &balancear)
...
void ArvoreAVL::imprime()
{
    ...
    cout << "altura da arvore -> " << calcAltura(this->raiz) << endl;
}

int ArvoreAVL::calcAltura(NoAVL *raiz)
{
    if (raiz != NULL)
    {
        //recursao
        int altEsq = calcAltura(raiz->sub[0]);
        int altDir = calcAltura(raiz->sub[1]);
